...
{-| A `Dict` contains all the `CountryData` used in the default `Config`. Use this if you need to override some `CountryData` value, and then use it on your `Config`.

See also: [countryList](#countryList)
...
{-| A list tuple of country's ISO codes and `CountryData`.

See also: [countries](#countries)
